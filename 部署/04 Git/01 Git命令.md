...
先复习Git的几个工作区域哈：![图片](C:\学无止境\Image\01-git-04)
...
![图片](C:\学无止境\Image\01-git-05)git 的正向工作流程一般就这样：
...
这个图只是模拟一下git基本命令使用的大概流程哈~![图片](C:\学无止境\Image\01-git-08)
...
当你忘记是否已把代码文件添加到暂存区或者是否提交到本地仓库，都可以用git status看看哦~![图片](C:\学无止境\Image\01-git-13)
...
嘻嘻，看看dev分支上的提交历史吧~要回滚代码就经常用它喵喵提交历史~![图片](C:\学无止境\Image\01-git-14)
...
如果你想对比一下你改了哪些内容，可以用git diff对比一下文件修改差异哦![图片](C:\学无止境\Image\01-git-15)
...
如果我们在dev开发完，或者就想把文件推送到远程仓库，给别的伙伴看看，就可以使用git push origin dev~![图片](C:\学无止境\Image\01-git-17)
...
比如，你开发完需求后，发版需要把代码合到主干master分支，如下：![图片](C:\学无止境\Image\01-git-22)
...
git merge提示冲突后，我们切换到对应文件，看看冲突内容哈，，如下：![图片](C:\学无止境\Image\01-git-24)
...
![图片](C:\学无止境\Image\01-git-25)

## **Git进阶之撤销与回退**

Git的撤销与回退，在日常工作中使用的比较频繁。比如我们想将某个修改后的文件撤销到上一个版本，或者想撤销某次多余的提交，都要用到git的撤销和回退操作。

代码在Git的每个工作区域都是用哪些命令撤销或者回退的呢，如下图所示：![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==)
...
以下demo，使用git checkout -- test.txt 撤销了test.txt的修改![图片](C:\学无止境\Image\01-git-26)
...
![图片](C:\学无止境\Image\01-git-27)

假设执行git reset，回退到版本二之后，版本三不见了哦,如下：

![图片](C:\学无止境\Image\01-git-28)

#### git reset的使用

Git Reset的几种使用模式![图片](C:\学无止境\Image\01-git-29)
...
**revert之前：**![图片](C:\学无止境\Image\01-git-31)**revert 之后：**![图片](C:\学无止境\Image\01-git-32)
...
![图片](C:\学无止境\Image\01-git-35)

### git reflog

显示当前分支的最近几次提交

![图片](C:\学无止境\Image\01-git-36)

### git blame filepath

git blame 记录了某个文件的更改历史和更改人，可以查看背锅人，哈哈![图片](C:\学无止境\Image\01-git-37)
